...
회원가입과 로그인의 UI에서 겹치는 부분들을 auth_form, auth_input으로 컴포넌트화했습니다. 투두페이지에서는 반복되는 입력받는 input과 버튼들로 같은 UI로 만들기 위해서 컴포넌트화 했습니다. 



### 1.2 네트워크

네트워크 폴더에는 fetch사용과 비동기 코드의 중복을 줄이기위해 만든 httpClient 클래스와 서버의 응답에 따라 적절한 메시지를 보내주기 위한 httpError 클래스 두가지를 담고 있습니다. 



## 1.3 service

API통신을 위한 authService 클래스와 todoService 클래스를 담고 있습니다.



## 2. Auth페이지

Auth는 회원가입과 로그인을 한 페이지에서 나타냈습니다. 
...
상태는 loginInfo, registerInfo, registerMessage, LoginMessage 2가지 종류 info와 message 상태를 가지고 있습니다.



info 상태는 사용자가 입력한 input의 데이터를 보관하고 유효한지 아닌지 체크하는데 사용합니다.

message는 서버의 응답에 따라 성공과 실패 각각에 알맞은 메시지를 담고 있습니다.
...
함수들은 hanldeChange, handleLoginSubmit과 handleRegisterSubmit 3가지 종류로 생각할 수 있습니다.



#### 2.2.1 handleChange

handleChange는 회원가입과 로그인의 input창에서 발생한 이벤트의 값을 동일하게 해당 상태로 업데이트를 해주는 함수입니다.

이때 과제의 유효성검사를 위해서 이메일은 @를 포함했는지 비밀번호는 8자이상인지에 따라 true/false로 업데이트 해주고

모두 true일 때 제출버튼이 활성화됩니다.



(코드의 setInfo는 로그인의 auth_form과 회원가입 auth_form에 전달된 상태변경 함수를 의미합니다.)
...
submit함수는 로그인과 회원가입이 조금 다른 일을 처리해서 둘을 다르게 처리했습니다. 버튼을 누르면 전달된 상태를 받아 api를 호출합니다. api를 호출하기 위해 전달받은 authService의 메소드를 이용합니다.
...
로그인 성공으로 저장한 토큰의 유무에 따라 router에서 전체적인 페이지 이동을 위한 로직을 추가해 자동으로 redirect되게 설정했습니다.
