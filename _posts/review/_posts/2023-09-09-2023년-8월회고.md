...
드디어 8월이 지나가버렸다. 시간이 너무 빠르게 흘러 개발자로 일한지 벌써 6개월이란 시간이 지나버렸다. 이번 한달은 스쿼드의 인원변동이 있어서 업무보다는 모바일 챕터 업무에 많은 시간을 쏟았다. 7월 회고에 언급했던 **예외/에러 처리** 고도화 작업과 함께 React native 발표 자료들을 뒤적거리다 찾게된 성능 측정 툴인 **Flash Light** 발표 내용을 정리해서 챕터 기술 세션에 발표하기도 했다. 두가지 큰 작업들을 정리하면서 내가 어떤 점이 부족했는지, 어떤 점은 잘했다고 생각하는지 돌아보려 한다.
...
버그 스낵을 알아보면서 생소했던 개념은 *breadcrumbs*이라는 메소드인데 버그스낵의 짧은 로그로 다음과 같은 기록을 각 에러 로그에 자동으로 남김으로써 디버깅을 도와준다.
...
이 기준은 더 다양한 케이스들을 보고 챕터원들과 의논을 통해 계속해서 확장해 나갈 예정이다. 
 ...
 버그 스낵에 있는 수많은 로그들을 보면서 압도되기도 했지만 하나하나 보면서 우리 앱에서 놓치고 있는 부분이나 빠져있는 부분들을 보기도 하고, Native 에러를 보면서 모바일 개발자로써 더 많이 공부하고 싶다는 의욕도 생기는 계기가 되었다. 더 튼튼하고 사용성 좋은 앱으로 만들어가는 데 많은 기여를 할 수 있는 일감이 되기를 바라면서 나 또한 더 성장하는 계기가 되어가고 있다.
...
<video src="https://github.com/bamlab/flashlight/assets/4534323/4038a342-f145-4c3b-8cde-17949bf52612"/>
...
참고로 앱의 bundle Id는 `npx @perf-profiler/profiler getCurrentApp` 를 통해서 구할 수 있다.
...
<video src="https://github.com/choi2021/choi2021.github.io/assets/80830981/f4710c25-7d0a-4dec-bb81-084133be0929"></video>
