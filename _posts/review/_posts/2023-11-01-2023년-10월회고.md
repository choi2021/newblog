...
```
 ...
 위의 코드는 코드리뷰를 통해서 우리 프로젝트에서 성능측정을 위해서 사용하기 위해서 확장성있는 클래스로 다시 구현한 성능측정 유틸이다. 처음은 단순 클래스로 구현해 필요한 부분에서 instance를 이용해서 만들려했지만 스코프의 한계로 사용처의 문제점이 생길 수 있어, 싱글톤 패턴을 이용해 이전처럼 스코프와 상관없이 유틸을 사용하게 했고, Map을 이용해 start시 새로운 Trace를 할당하고, stop시에는 Trace를 삭제해 불필요한 메모리를 차지하지 않으면서 앞서 겪었던 두가지 문제점을 해결할 수 있었다. 
