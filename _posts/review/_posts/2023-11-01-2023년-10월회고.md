 ...
 **Firebase Performance**는 리액트 네이티브에서 이용하기 위해서 [React-native-firebase](https://github.com/invertase/react-native-firebase)를 이용할 수 있다. 기존 우리 프로젝트는 안드로이드와 ios 프로젝트로 구분해서 데이터를 쌓고 있었고, 기존 화면 렌더링 속도를 측정하기 위해 존재하던 유틸 코드를 참고해 코드를 작성하려했다. 

[프로젝트내 사용하고 있던 화면 TTI 측정 유틸]

```typescript
import perf from '@react-native-firebase/perf';

const trace = perf().newTrace('측정화면');
...
```
 ...
 위 코드를 보았을 때 견적발송를 측정하기에 충분해 보였고, 여기에 견적발송가능여부를 확인하는 useCase가 끝나는 데까지 걸리는 시간을 측정하기 위해서 중간지점에 대한 정보를 기록할수있는 함수를 추가하려 했다. 



[견적 발송 측정 util에 대한 첫 PR ]
...
const startTrace = async () => {
    await trace.start();
    // 임시로 시작점에 대한 정보를 기록하기 위해 추가한 metric
    trace.putMetric('start', new Date().getTime());
};

const stopTrace = async () => {
    // 임시로 시작점에 대한 정보를 얻기 위해 추가한 metric이기 대문에 제거
...
[Firebase Performance Web의 예제]

<img src="https://github.com/choi2021/choi2021.github.io/assets/80830981/922e59d0-8c6b-446c-b1d1-44fa5971abd6" />
...
class PerformanceTracker {
    ...
    private constructor(
        ...
        private startTimeMap: Map<keyof typeof PerformanceKey, number> = new Map(),
    ) {}

    static getInstance() {
        if (isNil(PerformanceTracker.instance)) {
    ...
    async start(name: keyof typeof PerformanceKey) {
        ...
        this.startTimeMap.set(name, Date.now());
    ...
    async stop(name: keyof typeof PerformanceKey) {
        ...
        if (isNil(trace) || isNil(startTime)) {
            console.debug(`${name} 측정 시작 안됨`);
            return;
        }
        await trace.stop();
        console.debug(`${name} 측정 완료: ${formatNumber(Date.now() - startTime)}ms`);
    ...
    addCheckPoint(name: keyof typeof PerformanceKey, metricName: string) {
        ...
        console.debug(`${name}  ${metricName} 체크포인트: ${formatNumber(checkPointTime)}ms`);
    }
}
...
```
 ...
 위의 코드는 우리 프로젝트에서 성능측정을 위해서 사용하기 위해서 확장성있는 클래스로 다시 구현한 성능측정 유틸이다. 처음은 견적발송 도메인에 한정한 단순 클래스로 구현해 필요한 부분에서 instance를 이용해서 만들려했지만, 코드리뷰를 통해 조금더 확장성 있는 유틸로 구현했다. 

유틸은 스코프의 한계로 instance에 접근가능한 스코프 내에서만 사용할 수 있는 한계점이 생길 수 있어, 싱글톤 패턴을 이용해 하나의 instance를 export해서 사용하게 구현했다. 내부적으로는 Map을 이용해 start시 새로운 Trace를 할당하고, stop시에는 Trace를 삭제해 불필요한 메모리를 차지하지 않으면서 앞서 겪었던 두가지 문제점을 해결했다. 
...
function use_견적발송(...arg){
  try{
    ...
    await performanceTracker.start(PerformanceKey.견적발송);
    if(견적발송_가능여부_체크_useCase1(필요정보1)===false){
      return false
    }
  	...

   	if(견적발송_가능여부_체크_useCaseN(필요정보N)===false){
      	return false
   	}
    
   	performanceTracker.addCheckPoint(PerformanceKey.견적발송, '견적발송_가능여부_체크');
    ...
    await performanceTracker.stop(PerformanceKey.견적발송);
   	return true
  	} catch(e){
      if(e instanceof 견적발송실패_에러케이스1){
        견적발송실패_에러케이스1에대한_예외처리1()
