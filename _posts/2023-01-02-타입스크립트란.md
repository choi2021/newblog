...
타입스크립트로 기본적인 타입을 정하고 사용하고 있지만, 매번 막히는 문제들이 있어 [이펙티브 타입스크립트](https://search.shopping.naver.com/book/catalog/32473346832)를 읽고 정리해나가려 한다. 
...
function calculateArea(shape: Shape) {
  if (shape instanceof Rectangle) { // 'Rectangle' only refers to a type, but is being used as a value here.
...
function calculateArea(shape: Shape) {
  if ('height' in shape) { // height이 있으므로 Rectangle로 결정
